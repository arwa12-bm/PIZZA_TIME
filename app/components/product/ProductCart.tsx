...
const ProductCart:React.FC<ProductCartProps> = ({data}) => {
    ...
    return ( 
    <div   className="col-span-1 h-full cursor-pointer border-[1.2px] border-slate-200 bg-slate-50 rounded-sm
    transition hover:scale-105  text-sm ">
        <div className="flex flex-col gap-2">
            <div className="aspect-square overflow-hidden relative w-full rounded-md h-[260px]">
            <Image fill src={data.image} alt="" />
            </div>
            ...
            <MdOutlinePayments size={40} className="rounded-3xl p-1 bg-green-600 text-white " />
            <LuBadgeDollarSign size={40} className="rounded-3xl p-1 bg-green-600 text-white" />
            <LuBadgeDollarSign size={40} className="rounded-3xl p-1 bg-green-600 text-white"/>
                </div>
